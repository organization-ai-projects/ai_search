...
pub fn gate_with_policy(scores: &[(ExpertId, f32)], pol: &GatingPolicy) -> Vec<(ExpertId, f32)> {
    let ws = softmax_stable(
        &scores.iter().map(|(_, s)| *s).collect::<Vec<_>>(),
        pol.temperature,
    );
    let mut v: Vec<(ExpertId, f32)> = scores
        .iter()
        .zip(ws)
        .map(|((id, _), w)| {
            (
                *id,
                (1.0 - pol.epsilon) * w + pol.epsilon / (scores.len() as f32),
            )
        })
        .collect();
